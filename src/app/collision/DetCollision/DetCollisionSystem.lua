...
function DetCollisionSystem.new(config)
    ...
    self.bodies = {}       -- id → {shape, group, mask, active}
...
------------------------------------------------------------------------
-- Register a collision callback: function(idA, idB, mtvX, mtvY)
------------------------------------------------------------------------
...
-- Each entry: {idA, idB, mtvX, mtvY}
-- MTV pushes idA away from idB.
--
...
------------------------------------------------------------------------
function DetCollisionSystem:detect()
    ...
    for i = 1, #candidatePairs do
        ...
        local bodyA = bodies[idA]
        local bodyB = bodies[idB]
        ...
        if bodyA and bodyB and bodyA.active and bodyB.active then
            ...
            if canCollide then
                local hit, mtvX, mtvY = SAT.test(shapeA, shapeB)
                if hit then
                    collisions[#collisions + 1] = { idA, idB, mtvX or 0, mtvY or 0 }
                end
            end
        end
    end
    ...
    if #self.callbacks > 0 then
        for i = 1, #collisions do
            ...
            for j = 1, #self.callbacks do
                self.callbacks[j](c[1], c[2], c[3], c[4])
            end
        end
    end
...
------------------------------------------------------------------------
function DetCollisionSystem:step()
    self:rebuild()
    return self:detect()
end
