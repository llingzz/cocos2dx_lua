...
local scheduler = require("app.tools.Scheduler")
...
function SocketTCP:setEventProtocol(__eventProtocol)
...
function SocketTCP:_disconnect()
	self.isConnected = false
	self.tcp:shutdown()
	if self.eventProtocol then self.eventProtocol:dispatchEvent({name=SocketTCP.EVENT_CLOSED}) end
end
...
function SocketTCP:_onConnected()
	...
	local __tick = function()
		while true do
			...
			if self.eventProtocol then self.eventProtocol:dispatchEvent({name=SocketTCP.EVENT_DATA, data=(__partial or __body), partial=__partial, body=__body}) end
		end
	end
...
function SocketTCP:_connectFailure(status)
	...
	if self.eventProtocol then self.eventProtocol:dispatchEvent({name=SocketTCP.EVENT_CONNECT_FAILURE}) end
	self:_reconnect();
end
